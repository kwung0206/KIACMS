...
export default function MentorStudentsPage() {
  ...
  async function handleAssignStudent() {
    ...
    try {
      ...
      setFeedback({ type: "success", message: "학생을 담당 목록에 배정했습니다." });
      setAssignmentMemo("");
  ...
  async function handleAssignCourse() {
    ...
    try {
      ...
      setFeedback({
        type: "success",
        message: "수업을 학생에게 매핑했습니다. 학생 캘린더에도 자동 반영됩니다.",
      });
      await loadData();
    } catch (error) {
      setFeedback({ type: "error", message: error.message });
  ...
  if (loading) {
    return <LoadingScreen message="학생 관리 화면을 준비하는 중입니다." />;
  }
  ...
  return (
    <div className="page-stack">
      <PageHeader
        title="담당 학생 관리"
        description="학생 검색, 담당 배정, 학생-수업 매핑을 한 화면에서 처리할 수 있습니다."
        actions={
          <button className="primary-button button-small" type="button" onClick={() => setStudentModalOpen(true)}>
            학생 선택
          </button>
        }
      />
      ...
      <section className="detail-grid">
        <article className="panel">
          <div className="section-title-row">
            <h2>담당 학생 목록</h2>
            <span className="muted-text">{managedStudents.length}명</span>
          </div>

          {managedStudents.length === 0 ? (
            <EmptyState
              title="아직 담당 학생이 없습니다."
              description="상단의 학생 선택 버튼을 눌러 담당 학생을 배정해 주세요."
            />
          ) : (
        ...
        <article className="panel">
          ...
          {!selectedManagedStudent ? (
            <EmptyState
              ...
              description="왼쪽 목록에서 관리할 학생을 선택해 주세요."
            />
          ) : (
      ...
      <section className="panel">
        ...
        {!selectedManagedStudent || selectedManagedStudent.enrolledCourses.length === 0 ? (
          <EmptyState
            title="매핑된 수업이 없습니다."
            description="위에서 수업을 선택해 학생과 연결해 주세요."
          />
        ) : (
          <div className="list-stack">
      ...
      {studentModalOpen ? (
        <div className="modal-backdrop" role="presentation" onClick={() => setStudentModalOpen(false)}>
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="mentor-student-modal-title"
            onClick={(event) => event.stopPropagation()}
          >
            <div className="modal-header">
              <div>
                <h2 id="mentor-student-modal-title">학생 선택</h2>
                <p className="muted-text">
                  검색 결과에서 학생을 선택한 뒤 담당 학생으로 배정할 수 있습니다.
                </p>
              </div>
            </div>
            ...
            {studentOptions.length === 0 ? (
              ...
              <div className="panel nested-panel form-stack">
                <div className="info-card">
                  <strong>{selectedStudentOption?.name || "학생을 선택해 주세요."}</strong>
                  <span>{selectedStudentOption?.email || "선택된 학생 정보가 없습니다."}</span>
                </div>
                ...
                <div className="button-row align-end">
                  <button className="ghost-button button-small" type="button" onClick={() => setStudentModalOpen(false)}>
                    닫기
                  </button>
                  <button
